...
class GlmVisionClient:
    ...
    def __init__(self, settings: Settings):
        ...
        self.base_url = str(settings.get("vision", "glm_base_url", "https://open.bigmodel.cn/api/paas/v4")).rstrip(
            "/"
        )
        self.model = str(settings.get("vision", "glm_model", "glm-4.1v-thinking-flash"))
        self.timeout_seconds = int(settings.get("vision", "timeout_seconds", 90))
        self.api_key = os.environ.get("ZHIPU_API_KEY", "").strip() or str(
            settings.get("vision", "glm_api_key", "")
        ).strip()
    ...
    def analyze_images(self, image_paths: list[Path], prompt: str) -> dict[str, Any]:
        ...
        if not text:
            raise RuntimeError("GLM vision returned empty output.")
        return parse_json_object(text, "GLM vision")
