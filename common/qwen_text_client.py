...
class QwenTextClient:
    ...
    def __init__(self, settings: Settings):
        ...
        self.model = str(settings.get("writer", "qwen_model", "qwen-plus-latest"))
        ...
        self.api_key = os.environ.get("DASHSCOPE_API_KEY", "").strip() or str(
            settings.get("writer", "qwen_api_key", settings.get("vision", "qwen_api_key", ""))
        ).strip()
    ...
    def generate_json(self, prompt: str, system_prompt: str) -> dict[str, Any]:
        if not self.is_configured():
            raise RuntimeError("DASHSCOPE_API_KEY is not configured for writer.")
        ...
        if not text:
            raise RuntimeError("Qwen writer returned empty output.")
        return parse_json_object(text, "Qwen writer")
